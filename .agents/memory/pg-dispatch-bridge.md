...
## Phase 5J-Tier-4 (Driver write-path inversion: ACCEPT + STAGE) — DONE, verdict READY_FOR_PHASE_5J_TIER_5
Driver App accept + delivery-stage writes moved from direct Firestore writes to PG-authoritative REST endpoints (POST /orders/:id/accept, PATCH /orders/:id/stage), with a best-effort Firestore *projection* preserved so the untouched Firestore FCM dispatcher + customer app keep working. Durable patterns:
- **Driver mutation inversion order = PG first (authoritative), then guarded Firestore projection.** PG is the arbiter (accept via existing atomic `pgAcceptOffer`; stage via new `pgSetOrderStage` guarded UPDATE on id + driver_uid + status NOT IN delivered/cancelled). Firestore mirror is explicitly best-effort and NEVER clobbers a terminal/cancelled doc (and stage mirror also re-checks driverUid match). **Why:** cancel stays Firestore-authoritative this tier, so a guarded projection accepts a small, logged PG/Firestore divergence window rather than resurrecting a cancelled order. Divergence is acceptable at this tier because PG is now source of truth.
- **Duplicate-accept idempotency lives at the route, not in pgAcceptOffer.** When `pgAcceptOffer` returns `already_claimed`, the route re-reads `pgGetOrder`; if the order is already owned by the SAME driver and in an active status, it returns ok:true (idempotent). Different driver → real already_claimed. Prevents a fast double-tap / notification+in-app double accept from showing a false failure to the winning driver.
- **Stage endpoint only ever handles 4 stages (to_pickup/at_pickup/to_drop/at_drop).** at_drop→delivered is intercepted earlier by the OTP gate in active-delivery.tsx and goes through POST /complete; so `delivered` is intentionally rejected (400 invalid_stage) by /stage and absent from STAGE_TIMESTAMP_KEY. Don't "add delivered support" to /stage — it would bypass OTP/wallet credit.
- **Scope discipline:** location writes (`updateDriverLocation`), `driverCancelOrder`, reject/timeout, realtime listeners stay on Firestore — only accept + stage inverted. Mobile helpers (`acceptOrderViaApi` returns AcceptOrderResult; `updateOrderStageViaApi` void fire-and-forget) map PG reasons→client vocab so call sites are unchanged.
- Safety env unchanged (DISPATCH_SOURCE=pg, ALLOW_PG_DISPATCH_WRITES=true, PG_PROJECTION_ENABLED=true, PG_FCM_SEND_ENABLED=false). Both typechecks + api build clean; restart OK; /accept + /stage gate to 401 (not 404); PG dispatcher healthy + FCM still blocked. Architect: no blockers. Optional future hardening: projection-failure telemetry/alerts + a stage-projection reconciliation/retry path.

## Phase 5J-Tier-5 (Driver write-path inversion: STATUS + LOCATION) — DONE, verdict READY_FOR_PHASE_5J_TIER_6_SSE
Driver online/offline status and active-delivery live location moved to PG-authoritative + best-effort Firestore projection, completing the Tier-4 pattern for the remaining SAFE driver writes. Durable patterns:
- **Online status flipped to PG-PRIMARY — this INVERTS the Phase 4C shadow model.** `PATCH /drivers/:uid/status` now writes PG first (404 on missing drivers row, 500 on PG failure), then projects to Firestore non-blocking (`[PG_STATUS_PROJECTION]`/`[PG_STATUS_PROJECTION_FAILED]`). The old Phase-4C "Firestore-first + PG shadow + accepted auto-offline shadow-lag" is GONE: all 4 mobile call sites (plan-expiry auto-offline, account-block enforcement, signOut, setOnline) now call `patchDriverStatus` only; the setOnline dual-write was removed. **Why safe:** in pg mode nothing customer-facing reads Firestore `drivers.isOnline` (RR is guarded off; customer app doesn't read it; driver app always cold-starts offline), so the Firestore mirror is rollback/observability only.
- **Two DISTINCT location writes — only the order-doc one was migrated.** `POST /drivers/:uid/location` (the ~15s while-online driver-doc ping, Phase 4D) stays Firestore-primary+PG-shadow, UNCHANGED. The migrated one is the per-order customer-live-map write: new `pgSetOrderLocation` (guarded atomic UPDATE id+driver_uid+status NOT IN delivered/cancelled, numeric cols as strings, classifies not_found/forbidden/terminal — mirrors pgSetOrderStage) behind new `PATCH /orders/:orderId/location`, PG-first then best-effort Firestore projection of driverLat/driverLng/locationUpdatedAt/locationAccuracy (`[PG_LOCATION_PROJECTION]`). Mobile GPS watcher in active-delivery.tsx swapped `updateDriverLocation` → fire-and-forget `updateOrderLocationViaApi`.
- **High-frequency write tolerance:** location fires ~1/10s/active-order; a dropped PG write or missed Firestore mirror self-heals on the next tick, so the client helper swallows all errors and the projection never blocks the response. orders schema: driver_lat numeric(10,8), driver_lng numeric(11,8) — India lng<100 so no overflow.
- **Deferred (dispatcher-coupled, NOT migrated this tier):** reject/timeout (`arrayRemove activeOfferDriverUids`) + `driverCancelOrder` stay Firestore-authoritative to respect "do not change PG dispatcher authority". Payout + order-complete/earnings were already PG-authoritative.
- Safety env unchanged; both typechecks + api build clean; restart OK; /location + /status gate to 401 (not 404); PG dispatcher healthy (processed=applied, 0 failed); FCM still only from Firestore dispatcher (pgMessageId=null). Architect: no blockers.
