// @ts-nocheck — build-time config plugin; @expo/config-plugins is a transitive
// dependency of expo and is available to Expo's prebuild toolchain but is not
// declared as a direct dependency in this package's package.json, so TypeScript
// cannot resolve its types from here.
 ...
 * • No SYSTEM_ALERT_WINDOW permission is requested or required.
 */
...
function withFullScreenOrderAlert(config) {
  ...
  config = withAndroidManifest(config, (mod) => {
    ...
    if (app) {
      ...
      const alreadyRegistered = receivers.some((r) =>
        String(r.$?.["android:name"]).includes("FullScreenOrderActionReceiver"),
      );
