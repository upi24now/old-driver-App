...
export default function ActiveDeliveryScreen() {
  ...
  // 4-digit OTP generated by the customer app and stored on the Firestore order doc.
  // Undefined for orders created before OTP support was added (legacy orders).
  const expectedOtp = activeRide?.deliveryOtp;
  ...
  function advance() {
    ...
    if (stage === "at_drop") {
      ...
      // Legacy order (deliveryOtp absent): 4-digit entry is accepted without match check.
    }
  ...
  return (
    <View style={[st.root, { paddingTop: insets.top }]}>
      ...
      >
        ...
        <View style={[st.csSheet, { paddingBottom: insets.bottom + 16 }]}>
          <View style={st.csDragPill} />
          <Text style={st.csTitle}>Cancel Order</Text>
          <Text style={st.csSub}>Why are you cancelling this order?</Text>

          {([
            { emoji: "⚠️", label: "Route Not Reachable",      sub: "Road blocked or inaccessible",   reason: "Route Not Reachable"      },
            { emoji: "🚗", label: "Vehicle Breakdown",         sub: "Vehicle issue or accident",       reason: "Vehicle Breakdown / Accident" },
            { emoji: "📦", label: "Oversized Parcel",          sub: "Parcel too large for vehicle",    reason: "Oversized Parcel"         },
          ] as const).map(({ emoji, label, sub, reason }) => (
            <TouchableOpacity
              key={reason}
              activeOpacity={0.8}
              style={st.csCard}
            ...
            >
              <Text style={st.csCardEmoji}>{emoji}</Text>
              <View style={st.csCardText}>
                <Text style={st.csCardLabel}>{label}</Text>
                <Text style={st.csCardSub}>{sub}</Text>
              </View>
              <Feather name="chevron-right" size={18} color="#94A3B8" />
            </TouchableOpacity>
          ))}

          <TouchableOpacity style={st.csKeep} activeOpacity={0.7} onPress={() => setCancelVisible(false)}>
...
const st = StyleSheet.create({
  ...
  // Cancel bottom sheet
  csBackdrop:    { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(0,0,0,0.52)" },
  csSheet:       { position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: "#fff", borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 20, paddingTop: 12, gap: 10 },
  csDragPill:    { alignSelf: "center", width: 36, height: 4, borderRadius: 2, backgroundColor: "#E2E8F0", marginBottom: 4 },
  csTitle:       { fontSize: 18, fontWeight: "900", color: "#0F172A", textAlign: "center" },
  csSub:         { fontSize: 13, color: "#64748B", textAlign: "center", marginBottom: 4 },
  csCard:        { flexDirection: "row", alignItems: "center", gap: 14, backgroundColor: "#F8FAFC", borderRadius: 16, borderWidth: 1, borderColor: "#E2E8F0", paddingHorizontal: 16, paddingVertical: 14 },
  csCardEmoji:   { fontSize: 26, width: 36, textAlign: "center" },
  csCardText:    { flex: 1, gap: 2 },
  csCardLabel:   { fontSize: 15, fontWeight: "700", color: "#0F172A" },
  csCardSub:     { fontSize: 12, color: "#64748B" },
  csKeep:        { alignItems: "center", paddingVertical: 14 },
  csKeepTxt:     { fontSize: 15, fontWeight: "700", color: "#94A3B8" },
});
