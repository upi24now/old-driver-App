/**
 * ride-flow-demo.tsx — TEMPORARY DEMO / PREVIEW SCREEN (safe to delete)
 * ============================================================================
 * Purpose: visually preview the Driver active-ride flow WITHOUT any real
 * booking, dispatch, OTP, wallet, earnings or backend API calls.
 *
 * This screen is 100% self-contained and LOCAL:
 *   - NO imports from utils/delivery-api, utils/firestore, utils/support
 *   - NO useDriver() / DriverContext usage (does not touch real dispatch/orders)
 *   - NO expo-location / expo-network (no real GPS / connectivity required)
 *   - Fixed demo OTP = "1234"; completion is local-only (no /complete call)
 *
 * HOW TO REMOVE THIS DEMO LATER (one small patch):
 *   1. Delete this file: artifacts/mobile/app/ride-flow-demo.tsx
 *   2. Remove the "Ride Flow Demo" dev button block in
 *      artifacts/mobile/app/(tabs)/index.tsx (search: "RIDE FLOW DEMO (TEMP)")
 *   3. Remove the "/ride-flow-demo" entries in .expo/types/router.d.ts
 *      (regenerated automatically by the dev server anyway)
 * ============================================================================
 */
...
// ─── DEMO config ──────────────────────────────────────────────────────────────
const DEMO_OTP = "1234";
type PayMode = "cod" | "online";

// One fake LOCAL order (never a real booking) ─ supports COD + Online preview.
const DEMO_ORDER = {
  id:         "DEMOORDER",
...
// DEMO: navigation just opens Maps (safe, no permission/location). Failure is
// swallowed silently so the demo keeps working offline / on web.
function openGoogleMapsSafe(address: string, city: string) {
...
// ─── At-drop delivery card ────────────────────────────────────────────────────
function DeliverCard({ drop, dropCity, customer, phone, earning, emoji, parcel }: {
  drop: string; dropCity: string; customer: string; phone: string;
    ...
    <View style={[st.card, { borderColor: "#8B5CF640" }]}>
      <View style={[st.confirmBanner, { backgroundColor: "#EDE9FE" }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={st.confirmSub}>Collect payment, then confirm with OTP</Text>
        </View>
      </View>
      <View style={st.parcelDetailRow}>
        <Text style={{ fontSize: 32 }}>{emoji}</Text>
...
function RatingCard({ rating, onRate }: { rating: number; onRate: (n: number) => void }) {
  return (
    <View style={st.card}>
      <Text style={st.rateTitle}>Rate the customer</Text>
      <Text style={st.rateSub}>How was your experience? (demo only)</Text>
      <View style={st.rateStars}>
        {[1, 2, 3, 4, 5].map((n) => (
          <TouchableOpacity key={n} activeOpacity={0.7} onPress={() => { haptic("light"); onRate(n); }}>
            <Feather name="star" size={34} color={n <= rating ? ORANGE : "#CBD5E1"} />
          </TouchableOpacity>
        ))}
      </View>
      {rating > 0 && <Text style={st.rateThanks}>Thanks! You rated {rating} star{rating > 1 ? "s" : ""}.</Text>}
    </View>
  );
}

// ─── Main demo screen ─────────────────────────────────────────────────────────
export default function RideFlowDemoScreen() {
  ...
  // Payment-first at drop: open Payment sheet; OTP only after payment confirmed.
  useEffect(() => {
    if (stage === "at_drop") {
  ...
  function resetDemo() {
    ...
    setPickupNavigated(false); setDropNavigated(false);
    setPaymentVisible(false); setPaymentConfirmed(false);
    setOtpSheetVisible(false); setRating(0);
  }

  const handleCall      = () => Alert.alert("Demo", "Calling is disabled in demo mode.");
  const handleNavPickup = () => { openGoogleMapsSafe(o.pickup, o.pickupCity); setPickupNavigated(true); };
  const handleNavDrop   = () => { openGoogleMapsSafe(o.drop, o.dropCity);     setDropNavigated(true);   };

  // Mark Pickup → to_pickup → at_pickup (local only, no API).
  function markPickup() {
    haptic("success");
    setStage("at_pickup");
  }

  // Confirm pickup → at_pickup → to_drop (drop leg begins; local only).
  function headToDrop() {
    haptic("success");
    setDropNavigated(false);
    setStage("to_drop");
  }

  // Mark Drop → to_drop → at_drop (local only, no API, no completion).
  function markDrop() {
    haptic("success");
    setStage("at_drop");
  }
  ...
  function onCashCollected() {
    ...
    Alert.alert(
      ...
      `Have you received full ₹${o.fare} from the customer?`,
      [
  ...
  // DEMO OTP check — local only. Correct OTP completes the ride; wrong = error.
  function submitOtp() {
    if (otp.length !== 4) { setOtpError("Enter 4-digit OTP"); return; }
    if (otp !== DEMO_OTP) { setOtpError(`Incorrect OTP. Use ${DEMO_OTP} for demo.`); return; }
    haptic("success");
    setOtpError(null);
    setOtpSheetVisible(false);
    setStage("delivered");
  ...
  const cta: { label: string; icon: string; color: string; hint?: string; onPress: () => void } = (() => {
    ...
    if (stage === "at_pickup") {
      return { label: "Confirm Pickup & Head to Drop", icon: "arrow-right", color: GREEN, onPress: headToDrop };
    }
    if (stage === "to_drop") {
    ...
    <View style={{ flex: 1, backgroundColor: BG }}>
      ...
      <View style={[st.topBar, { paddingTop: insets.top + 10 }]}>
        <View style={st.topBarLeft}>
          <TouchableOpacity style={st.topBarBackBtn} onPress={() => router.back()} activeOpacity={0.7}>
            <Feather name="arrow-left" size={18} color={TEXT} />
          </TouchableOpacity>
          <View>
            <Text style={st.topBarTitle}>Order #{o.id.slice(-6).toUpperCase()}</Text>
      ...
      <View style={st.demoBadge}>
        ...
        <Text style={st.demoBadgeTxt}>DEMO MODE — No real booking</Text>
        <TouchableOpacity onPress={resetDemo} activeOpacity={0.7} style={st.demoReset}>
          <Feather name="refresh-cw" size={12} color="#92400E" />
          <Text style={st.demoResetTxt}>Reset</Text>
        </TouchableOpacity>
      </View>
      ...
      >
        {/* Payment-mode preview toggle (demo control) */}
        ...
        <Animated.View style={{ gap: 12, transform: [{ translateY: cardY }], opacity: cardOpac }}>
          ...
          {stage === "delivered" && (
            <>
              <CelebrationCard earning={earning} customer={o.customer} distKm={o.distKm} />
              <RatingCard rating={rating} onRate={setRating} />
            </>
          )}
        </Animated.View>
      </ScrollView>
          ...
          <Pressable style={[st.otpSheet, { paddingBottom: insets.bottom + 16 }]} onPress={() => {}}>
            ...
            <View style={[st.payAmountBox, { borderColor: (isCod ? PRIMARY : GREEN) + "30", backgroundColor: (isCod ? PRIMARY : GREEN) + "0D" }]}>
              ...
              <Text style={st.payAmountHint}>
                {isCod ? `Collect ₹${o.fare} from customer` : "Payment already received"}
              </Text>
            </View>
            <TouchableOpacity
            ...
            >
              ...
              <Text style={[st.deliverBtnText, { color: "#fff" }]}>
                {isCod ? "Collect Cash & Continue" : "Continue to OTP"}
              </Text>
            </TouchableOpacity>
            <Text style={st.otpNote}>
              {isCod ? "⚠️ Confirm only after receiving the full cash amount" : "Tap continue to proceed to delivery OTP"}
            </Text>
          </Pressable>
        </Pressable>
      </Modal>

      {/* ── OTP BOTTOM SHEET (demo OTP = 1234) ──────────────────────────────── */}
      <Modal
          ...
          <Pressable style={[st.otpSheet, { paddingBottom: insets.bottom + 16 }]} onPress={() => {}}>
            ...
            <View style={st.otpSheetHeader}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={st.otpSheetSub}>Demo OTP is {DEMO_OTP}</Text>
              </View>
            ...
            <Text style={st.otpNote}>⚠️ Demo only — enter {DEMO_OTP} to complete locally</Text>
          </Pressable>
        </Pressable>
      </Modal>
    </View>
...
// ─── Styles (mirrors active-delivery.tsx) ─────────────────────────────────────
const st = StyleSheet.create({
  ...
  earningPill: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, borderWidth: 1 },
  ...
  demoBadgeTxt: { flex: 1, fontSize: 12, fontWeight: "800", color: "#92400E", letterSpacing: 0.3 },
  demoReset: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#FDE68A", paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 },
  demoResetTxt: { fontSize: 11, fontWeight: "800", color: "#92400E" },
  toggleCard: { backgroundColor: CARD, borderRadius: 14, padding: 12, borderWidth: 1, borderColor: BORDER, gap: 8 },
  toggleLabel: { fontSize: 11, fontWeight: "700", color: MUTED, textTransform: "uppercase", letterSpacing: 0.5 },
  toggleRow: { flexDirection: "row", gap: 8 },
  toggleBtn: {
    flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6,
    paddingVertical: 10, borderRadius: 10, borderWidth: 1.5, borderColor: BORDER, backgroundColor: BG,
  },
  ...
  routeCard: {
    backgroundColor: CARD, borderRadius: 16, padding: 14, gap: 12, borderWidth: 1.5,
  ...
  rateTitle: { fontSize: 16, fontWeight: "800", color: TEXT, textAlign: "center" },
  rateSub: { fontSize: 12, color: MUTED, textAlign: "center" },
  rateStars: { flexDirection: "row", justifyContent: "center", gap: 10, paddingVertical: 4 },
  rateThanks: { fontSize: 12, fontWeight: "700", color: GREEN, textAlign: "center" },
  ...
  ctaBtn: {
    height: 60, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, borderRadius: 18,
  ...
  deliverBtn: {
    height: 56, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, borderRadius: 16,
  },
