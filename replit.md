...
- **PIN routes coexist with OTP routes** — `POST /api/auth/set-pin` (Firebase Bearer-gated) and `POST /api/auth/verify-pin` (phone + 6-digit PIN → same `createCustomToken("91"+phone)` as verify-otp) live in `auth.ts` alongside the untouched OTP routes. PINs are hashed with Node built-in `node:crypto` scrypt (NOT bcrypt/argon2 — native addons break the esbuild bundle); only the hash is stored, raw PIN is never logged. verify-pin uses `SELECT ... FOR UPDATE` for a race-safe attempt counter; no PIN set returns 404 (client falls back to OTP). The `drivers` PIN columns are all nullable/defaulted so existing rows are unaffected. NOTE: the **Single-device login** entry below supersedes the login model — PIN is now the PRIMARY daily login (not "parallel/never a replacement") and the lock policy is 3 wrong attempts → 24h (not 5/15min).
- **Single-device login (PIN-primary)** — daily login is phone + 6-digit PIN (`POST /api/auth/verify-pin`); OTP is only for first-time PIN setup and forgot/reset, always routing to `/create-pin`. One account = one active device: every successful login (verify-otp, verify-pin, set-pin) mints `sessionId = crypto.randomUUID()` stored on `drivers.active_session_id` and returned to the client, which sends it back as the `x-session-id` header. The `__dsRequireDriver` gate (and the canonical `require-auth.ts`) enforces it: when `active_session_id` is non-NULL and ≠ header → `401 {error:"SESSION_REPLACED", message:"Logged in on another device"}`; the app clears auth and returns to login. Enforcement is **null-safe** (NULL session skips the check, so legacy rows keep working until next login) and **fail-open** on a session-check DB error (never hard-blocks a token-verified driver). OTP sends are capped 3/rolling-24h per phone via `otp_send_events` (test phones from `TEST_OTP_PHONES` bypass); PIN allows 3 wrong attempts then 24h lock (`FOR UPDATE` race-safe). PINs are scrypt `scrypt$<saltHex>$<hashHex>` in PG (same native-crypto rationale as below). Prod ships as a byte-safe additive bundle patch in `production-baseline/driver-session-login-patch/` (base `395ffcb2`, patched `21ee8ca7`); the block must splice AFTER the pino middleware so its `/api/auth/send-otp`+`/api/auth/verify-otp` overrides register BEFORE the base bundle's own pre-existing OTP override (first-match-wins).
