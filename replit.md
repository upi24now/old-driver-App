...
- **PIN login is parallel to OTP, never a replacement** — `POST /api/auth/set-pin` (Firebase Bearer-gated) and `POST /api/auth/verify-pin` (phone + 6-digit PIN → same `createCustomToken("91"+phone)` as verify-otp) live in `auth.ts` alongside the untouched OTP routes. PINs are hashed with Node built-in `node:crypto` scrypt in `src/lib/pin-hash.ts` (NOT bcrypt/argon2 — native addons break the esbuild bundle); only the hash is stored, raw PIN is never logged. verify-pin uses `SELECT ... FOR UPDATE` for a race-safe attempt counter: 5 wrong tries locks for 15 min (429), no PIN set returns 404 (client falls back to OTP). The `drivers` PIN columns are all nullable/defaulted so existing rows are unaffected.
